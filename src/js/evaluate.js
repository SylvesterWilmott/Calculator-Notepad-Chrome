            ...
            for (var b, c, d, f = [], m = a.length, n = 0; n < m; n++)
              if (!(n < m - 1 && " " === a[n] && " " === a[n + 1])) {
                for (
                  b = "",
                    c =
                      a.length - n > s.length - 2 ? s.length - 1 : a.length - n;
                  c > 0;
                  c--
                )
                  if (void 0 !== s[c])
                    for (d = 0; d < s[c].length; d++)
                      e(a, s[c][d], n, c) &&
                        ((b = s[c][d]), (d = s[c].length), (c = 0));
                if (((n += b.length - 1), "" === b))
                  throw new g.Exception("Can't understand after " + a.slice(n));
                var o = h.indexOf(b);
                f.push({
                  index: o,
                  token: b,
          ...
          (g.addToken = function(a) {
            for (var b = 0; b < a.length; b++) {
              ...
              -1 === d
                ? (h.push(a[b].token),
                  l.push(a[b].type),
                  s.length <= a[b].token.length && (s[a[b].token.length] = []),
                  s[a[b].token.length].push(a[b].token),
                  j.push(a[b].value),
                  i.push(a[b].show))
                : ((h[d] = a[b].token),
                  (l[d] = a[b].type),
                ...
                if (14 !== y.type) {
                  ...
                  for (z = k.length; z-- && 0 === k[z]; )
                    if (-1 !== [0, 2, 3, 4, 5, 9, 11, 12, 13].indexOf(B)) {
                      if (!0 !== s[B])
                        throw new g.Exception(A + " is not allowed after " + v);
                  ...
                  if (
                    ...
                    (s = m), (u = p), d(k, 2).push(2), j.push(w), j.push(i);
                  else if (1 === B)
                    1 === F.type ? ((F.value += C), d(k, 1)) : j.push(w),
                      (s = n),
                      (u = o);
                  ...
                  c < x.length - 1 &&
                  1 === x[c + 1].type &&
                  (1 === x[c - 1].type || 6 === x[c - 1].type)
                )
                  throw new g.Exception("Unexpected Space");
              }
      ...
      5: [
        function(a, b, c) {
          ...
          (d.prototype.postfixEval = function(a) {
            ...
            for (
              var b, c, e, f = [], g = this.value, h = void 0 !== a.n, i = 0;
              i < g.length;
              i++
            )
              1 === g[i].type
